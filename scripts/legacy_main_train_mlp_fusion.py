...
from utils.splits import build_random_folds, get_random_fold_indices
...
# =========================================================
# 3D / physicochemical ablation training
# - keep frozen scaffold split
# - keep aligned sample universe identical to the main line
# - remove the whole XTB / RDKit branch
# - main branch: MoLFormer + D-MPNN -> z_main -> scalar MLP
# =========================================================
...
class TrainConfig:
    ...
    folds_to_run_list: str = ""  # e.g. "1,2,3,4,5"
    device: str = ""             # e.g. "cuda:0"
    output_tag: str = "ablation_no3d"
...
def parse_args_to_config() -> TrainConfig:
    cfg = TrainConfig()
    parser = argparse.ArgumentParser(description="Train topology-only melting point ablation")

    for field in fields(TrainConfig):
        current = getattr(cfg, field.name)
        if isinstance(current, bool):
            parser.add_argument(f"--{field.name}", nargs="?", const=True, type=str_to_bool)
        else:
            parser.add_argument(f"--{field.name}", type=type(current))
...
def get_run_basename(timestamp: str, cfg: TrainConfig) -> str:
    parts = [
        "mp",
        cfg.output_tag.strip() if cfg.output_tag.strip() else "ablation_no3d",
        "frozen_scaffold",
        f"fold{cfg.n_folds}",
        f"bs{cfg.batch_size}",
        f"seed{cfg.seed}",
...
def load_frozen_split_manifest(
    ...
    if not os.path.exists(manifest_file):
        ...
        return build_manifest_from_frozen_npy(split_dir, smiles_list, n_folds)
...
def build_manifest_from_frozen_npy(split_dir: str, smiles_list: Sequence[str], n_folds: int) -> pd.DataFrame:
    assigned_val_fold = np.zeros(len(smiles_list), dtype=np.int64)
    seen_val_idx = set()
    for fold in range(1, n_folds + 1):
        val_file = os.path.join(split_dir, f"fold_{fold}_val_idx.npy")
        if not os.path.exists(val_file):
            raise FileNotFoundError(
                f"Frozen split manifest not found and fallback index file is missing: {val_file}"
            )
        val_idx = np.load(val_file).astype(int).tolist()
        bad_idx = [idx for idx in val_idx if idx < 0 or idx >= len(smiles_list)]
        if bad_idx:
            raise ValueError(f"Fold {fold} has out-of-range validation indices: {bad_idx[:5]}")
        overlap = seen_val_idx & set(val_idx)
        if overlap:
            raise ValueError(f"Validation indices are assigned to multiple folds: {sorted(overlap)[:5]}")
        seen_val_idx.update(val_idx)
        assigned_val_fold[val_idx] = fold
    ...
    is_none_scaffold = np.zeros(len(smiles_list), dtype=bool)
    if none_idx:
        bad_none_idx = [idx for idx in none_idx if idx < 0 or idx >= len(smiles_list)]
        if bad_none_idx:
            raise ValueError(f"Out-of-range none-scaffold indices: {bad_none_idx[:5]}")
        none_val_overlap = set(none_idx) & seen_val_idx
        if none_val_overlap:
            raise ValueError(f"None-scaffold indices appear in validation folds: {sorted(none_val_overlap)[:5]}")
        is_none_scaffold[none_idx] = True
        assigned_val_fold[none_idx] = 0

    return pd.DataFrame(
        {
            "sample_id": np.arange(len(smiles_list), dtype=np.int64),
...
def build_manifest_from_random_folds(smiles_list: Sequence[str], fold_indices: Sequence[Sequence[int]]) -> pd.DataFrame:
    assigned_val_fold = np.zeros(len(smiles_list), dtype=np.int64)
    seen_val_idx = set()
    for fold, indices in enumerate(fold_indices, start=1):
        fold_idx = [int(idx) for idx in indices]
        bad_idx = [idx for idx in fold_idx if idx < 0 or idx >= len(smiles_list)]
        if bad_idx:
            raise ValueError(f"Fold {fold} has out-of-range validation indices: {bad_idx[:5]}")
        overlap = seen_val_idx & set(fold_idx)
        if overlap:
            raise ValueError(f"Validation indices are assigned to multiple random folds: {sorted(overlap)[:5]}")
        seen_val_idx.update(fold_idx)
        assigned_val_fold[fold_idx] = fold

    if len(seen_val_idx) != len(smiles_list):
        missing = sorted(set(range(len(smiles_list))) - seen_val_idx)
        raise ValueError(f"Random split did not assign every sample to a validation fold: {missing[:5]}")
...
def validate_frozen_manifest_alignment(smiles_list: Sequence[str], manifest_df: pd.DataFrame) -> None:
    ...
    actual_ids = manifest_sorted["sample_id"].astype(int).tolist()
    ...
    if manifest_smiles != list(smiles_list):
        mismatch_idx = next(
            (i for i, (a, b) in enumerate(zip(manifest_smiles, smiles_list)) if a != b),
            None,
        )
        raise ValueError(
            f"Frozen split manifest smiles do not align with the current aligned dataset order. "
            f"First mismatch at sample_id={mismatch_idx}: "
            f"manifest={manifest_smiles[mismatch_idx]!r}, data={smiles_list[mismatch_idx]!r}"
        )
...
def load_frozen_fold_indices(split_dir: str, fold: int, total_samples: int) -> Tuple[List[int], List[int]]:
    ...
    if not os.path.exists(train_file) or not os.path.exists(val_file):
        val_npy = os.path.join(split_dir, f"fold_{fold}_val_idx.npy")
        if not os.path.exists(val_npy):
            missing = train_file if not os.path.exists(train_file) else val_file
            raise FileNotFoundError(f"Frozen split file not found: {missing}")
        val_idx = sorted(np.load(val_npy).astype(int).tolist())
        val_idx_set = set(val_idx)
        train_idx = sorted(set(range(total_samples)) - val_idx_set)
        none_file = os.path.join(split_dir, "none_idx.npy")
        none_idx = np.load(none_file).astype(int).tolist() if os.path.exists(none_file) else []
        if set(none_idx) & val_idx_set:
...
# =========================================================
def load_aligned_multimodal_data(data_dir: str) -> Tuple[np.ndarray, List[str]]:
    """
    Keep the aligned sample universe identical to the main line:
    - read multimodal_train.csv
    - filter rows with missing SMILES / MP
    - filter rows whose SMILES do not exist in XTB_train.pth
    Even though this ablation does not USE XTB features, it must keep the same sample universe
    so that the frozen split stays exactly comparable to the main result.
    """
    print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Loading aligned sample universe from {data_dir}...', flush=True)
    ...
    print(
        f"Skipped rows | missing target/smiles: {skipped_missing_target} | missing XTB: {skipped_missing_xtb}",
        flush=True,
    )
    return targets, aligned_smiles
...
class JointMolGraphTextDataset(Dataset):
    def __init__(
        ...
        self.sample_ids = np.asarray(
            sample_ids if sample_ids is not None else np.arange(len(self.targets)),
            dtype=np.int64,
        )
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        graph = (
            ...
            else smiles_to_pyg_data(self.smiles_list[idx], use_dummy_on_failure=True)
        )
        return {
...
class MainOnlyScalarHead(nn.Module):
    """
    Ablation head:
    - keep the original z_main bottleneck (readout_dim = 16)
    - remove the whole XTB / RDKit modulation branch
    - extend z_main with a scalar MLP regressor
    """
    def __init__(
        self,
        bert_dim: int,
        ...
        self.scalar_head = nn.Sequential(
            nn.Linear(readout_dim, scalar_hidden_dim),
    ...
    def forward(self, bert_feat: torch.Tensor, dmpnn_feat: torch.Tensor, return_parts: bool = False) -> torch.Tensor:
        ...
        pred = self.scalar_head(z_main).squeeze(-1)
        ...
        if return_parts:
            return {
                ...
                "pred_scalar": pred.detach(),
                "z_main_l2_per_sample": torch.norm(z_main, dim=-1).detach(),
            }
        return pred


class JointMolFormerDMPNNMainOnlyModel(nn.Module):
    ...
    def forward(
        ...
        return self.head(bert_feat, dmpnn_feat, return_parts=return_parts)
...
def build_optimizer(model: JointMolFormerDMPNNMainOnlyModel, cfg: TrainConfig) -> optim.Optimizer:
    param_groups = [
        {
        ...
        },
        {
            ...
            "params": list(model.head.proj_bert.parameters()),
        ...
        },
        {
            ...
            "params": list(model.head.proj_dmpnn.parameters()) + list(model.head.main_head.parameters()) + list(model.head.scalar_head.parameters()),
            "lr": cfg.fusion_lr,
...
def apply_epoch_stage(model: JointMolFormerDMPNNMainOnlyModel, optimizer: optim.Optimizer, epoch: int, cfg: TrainConfig) -> str:
...
def train_one_epoch(
    model: JointMolFormerDMPNNMainOnlyModel,
    loader: DataLoader,
    criterion: nn.Module,
...
def validate(
    ...
    smiles_all: List[str] = []
    pred_scalar_std_list: List[np.ndarray] = []
    ...
    for batch in loader:
        ...
        pred_scalar_std_list.append(parts["pred_scalar"].cpu().numpy())
    ...
    pred_scalar_std = np.concatenate(pred_scalar_std_list, axis=0)[order]
    ...
    return {
        ...
        "pred_scalar_std": pred_scalar_std,
        "z_main_l2": z_main_l2,
    }
...
def build_prediction_frame(
    ...
    pred_df["pred_scalar_std"] = np.asarray(val_stats["pred_scalar_std"], dtype=np.float32)
...
def plot_fold_curves(history: Dict[str, List[float]], fold: int, output_dir: str) -> None:
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(epochs, history["train_pred_abs"], label="Train |pred|")
    plt.plot(epochs, history["val_pred_abs"], label="Val |pred|")
    plt.xlabel("Epoch")
    plt.ylabel("Average absolute prediction")
    plt.title(f"Fold {fold} Prediction Magnitude Curve")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"fold{fold}_prediction_magnitude_curve.png"), dpi=200)
    plt.close()
...
def run_training(cfg: TrainConfig, device: torch.device) -> None:
    ...
    print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Main-only ablation training script started', flush=True)
    ...
    targets_raw, smiles_list = load_aligned_multimodal_data(cfg.data_dir)
    ...
    if cfg.use_random_split:
        print(
            ...
            f"Using random K-fold split with seed {cfg.seed}",
            flush=True,
        )
        fold_indices_list = build_random_folds(total_sample_count, n_folds=cfg.n_folds, seed=cfg.seed)
        split_manifest = build_manifest_from_random_folds(smiles_list, fold_indices_list)
    else:
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Using frozen scaffold split from {split_dir}', flush=True)
        fold_indices_list = []
        split_manifest = load_frozen_split_manifest(split_dir, smiles_list=smiles_list, n_folds=cfg.n_folds)
    ...
    split_summary = {
        "run_id": run_id,
        "split_source": "random_kfold" if cfg.use_random_split else "frozen_scaffold",
        "split_dir": "" if cfg.use_random_split else split_dir,
    ...
    for fold in selected_folds:
        ...
        print("\n" + "=" * 80, flush=True)
        split_name = "random_kfold" if cfg.use_random_split else "frozen_scaffold"
        none_part = f" | None(train-only): {len(none_idx)}" if not cfg.use_random_split else ""
        print(
            f"Fold {fold}/{cfg.n_folds} | Split: {split_name} | "
            f"Train: {len(train_idx)} | Val: {len(val_idx)}{none_part}",
            flush=True,
        )
        print("=" * 80, flush=True)
        ...
        model = JointMolFormerDMPNNMainOnlyModel(cfg=cfg).to(device)
        ...
        if fold == selected_folds[0]:
            ...
            print(f"  BERT projection: {count_parameters(model.head.proj_bert):,}", flush=True)
            print(f"  D-MPNN encoder: {count_parameters(model.dmpnn_encoder):,}", flush=True)
            print(f"  D-MPNN projection: {count_parameters(model.head.proj_dmpnn):,}", flush=True)
            print(f"  Main head: {count_parameters(model.head.main_head):,}", flush=True)
            print(f"  Scalar head: {count_parameters(model.head.scalar_head):,}", flush=True)
            non_backbone = (
                count_parameters(model.head.proj_bert)
                + count_parameters(model.dmpnn_encoder)
                + count_parameters(model.head.proj_dmpnn)
                + count_parameters(model.head.main_head)
                + count_parameters(model.head.scalar_head)
            )
        ...
        for epoch in range(1, cfg.max_epochs + 1):
            ...
            print(
                ...
                f"|pred|(val): {val_stats['pred_abs']:.4f} | "
                ...
                f"dmpnn={lrs['dmpnn']:.2e}, common={lrs['common_fusion']:.2e} | "
                f"WD(common)={common_wd:.2e} | noise(z_main)={cfg.main_branch_noise_std:.4f}",
                flush=True,
            )
    ...
    print("\n========== Main-only Ablation Summary ==========", flush=True)
    ...
    print(f'\n[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Main-only ablation training completed.', flush=True)
