...
def get_scaffold(smiles, include_chirality=False):
    """获取分子的骨架（Murcko骨架）。如果是无环分子，则返回其标准化的SMILES"""
    try:
        # 清除首尾可能存在的不可见字符
        smiles = smiles.strip()
        ...
        # 如果提取出的 scaffold 对象为 None (极少见但可能发生)
        if scaffold is None:
        ...
        # 修复泄露的核心逻辑：处理无环直链分子返回空字符串的情况
        # 必须回退到该分子【标准化后】的SMILES，严禁返回原始未规范的字符串
        if not scaffold_smiles:
            return Chem.MolToSmiles(mol, isomericSmiles=include_chirality)
            
        return scaffold_smiles
    except Exception as e:
        # 遇到极其特殊的错误导致无法解析时，返回None直接丢弃该数据
        # 严禁将脏字符串直接当作骨架返回，否则必定引发泄露
        return None
...
def scaffold_split_for_multimodal(input_file, output_dir, test_size=4000, num_bins=10, seed=42):
    ...
    # 强制清理整个数据集SMILES列的隐形空格
    data['SMILES'] = data['SMILES'].astype(str).str.strip()
    print(f"总数据量: {len(data)} 条")
    
    smiles_list = data['SMILES'].tolist()
    
    # 提取骨架
    print("提取分子骨架并进行标准化去重...")
    ...
    for i, smi in enumerate(smiles_list):
        ...
        # 只有成功提取或标准化回退的分子才会进入划分池，过滤掉None（非法分子）
        if scaffold is not None:
            scaffold_to_indices[scaffold].append(i)
    
    unique_scaffolds = len(scaffold_to_indices)
    valid_molecules = sum(len(indices) for indices in scaffold_to_indices.values())
    print(f"提取完成! 共有 {unique_scaffolds} 个唯一骨架，包含 {valid_molecules} 个有效分子")
    
    # ---------------- 核心分布控制逻辑 ----------------
    
    # 1. 计算全局数据的 MP 分布和分位数边界
    # 仅使用成功提取骨架的有效分子的MP值
    valid_indices = [idx for indices in scaffold_to_indices.values() for idx in indices]
    valid_mps = data.iloc[valid_indices]['MP'].dropna()
